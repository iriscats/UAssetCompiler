            ...
            switch (kismetExpression)
            {
                ...
                case EX_LetValueOnPersistentFrame expr:
                {
                    var destinationProperty =
                        FormatString(_asset.GetPropertyName(expr.DestinationProperty, _useFullPropertyNames));
                    var assignmentExpression = FormatExpression(expr.AssignmentExpression, expr);
                ...
                case EX_StructMemberContext expr:
                {
                    ...
                    var structMemberExpression =
                        FormatIdentifier(_asset.GetPropertyName(expr.StructMemberExpression, _useFullPropertyNames));
                ...
                case EX_LocalFinalFunction expr:
                {
                    ...
                    var function = (FunctionExport)_asset.Exports
                    ...
                    {
                        var uberGraphFunctionLabel =
                            FormatCodeOffset((uint)firstParamInt.Value, function.ObjectName.ToString());
                    ...
                    else
                    {
                        ...
                        var isClassMemberFunction = (function?.OuterIndex.IsExport() ?? false) &&
                                                    (function?.OuterIndex.ToExport(_asset) == _class);
                    ...
                    switch (expr.Value.TextLiteralType)
                    {
                        ...
                        default:
                            throw new NotImplementedException(
                                $"EX_TextConst TextLiteralType {expr.Value.TextLiteralType} not implemented");
                    }
                }
                ...
                {
                    var innerProperty =
                        FormatIdentifier(_asset.GetPropertyName(expr.InnerProperty, _useFullPropertyNames));
                    var elements = string.Join(", ", expr.Elements.Select(x => FormatExpression(x, expr)));
                    if (!string.IsNullOrWhiteSpace(elements))
                    {
                ...
                {
                    return
                        $"EX_ArrayGetByRef({FormatExpression(expr.ArrayVariable, expr)}, {FormatExpression(expr.ArrayIndex, expr)})";
                }
                default:
